...
enum ResultItemRole {
    ...
    RoleScreenshotPath,
    RoleFullText,
    RoleOutputTargetPowerDbm,
    RoleOutputMeasuredPowerDbm,
    RoleOutputPowerErrorDb,
    RoleOutputFrequencyMHz,
    RoleOutputPeakFrequencyMHz,
    RoleOutputFrequencyErrorKHz,
    RoleOutputBandwidthText,
    RoleOutputSampleText
};
...
QVBoxLayout *resetChartBoxLayout(QGroupBox *box, const QString &title)
{
    ...
    while (QLayoutItem *item = oldLayout->takeAt(0)) {
        if (QWidget *w = item->widget()) {
            // 8.2 动态曲线会频繁刷新。这里只调用 deleteLater() 时，
            // 旧 QLabel / QChartView 会等事件循环空闲后才销毁，短时间内仍留在
            // groupBox 里绘制，容易和新图叠在一起，表现为标题、坐标轴文字重叠。
            // 先隐藏并脱离父控件，再延迟释放，可立即消除界面重叠。
            w->hide();
            w->setParent(nullptr);
            w->deleteLater();
        }
...
static double invalidOutputPowerValue()
{
    return std::numeric_limits<double>::quiet_NaN();
}

static bool outputPowerIsValid(double value)
{
    return std::isfinite(value);
}

static QString outputPowerNumberText(double value, int decimals)
{
    return outputPowerIsValid(value) ? QString::number(value, 'f', decimals) : QStringLiteral("--");
}

static double firstNumberWithUnit(const QString &text, const QString &unit)
{
    const QRegularExpression re(QStringLiteral("(-?\\d+(?:\\.\\d+)?)\\s*%1")
                                .arg(QRegularExpression::escape(unit)),
                                QRegularExpression::CaseInsensitiveOption);
    const QRegularExpressionMatch match = re.match(text);
    if (!match.hasMatch()) {
        return invalidOutputPowerValue();
    }
    bool ok = false;
    const double value = match.captured(1).toDouble(&ok);
    return ok ? value : invalidOutputPowerValue();
}

static double firstNumberAfterKeyword(const QString &text, const QString &keyword, const QString &unit)
{
    const QRegularExpression re(QStringLiteral("%1\\s*(-?\\d+(?:\\.\\d+)?)\\s*%2")
                                .arg(QRegularExpression::escape(keyword),
                                     QRegularExpression::escape(unit)),
                                QRegularExpression::CaseInsensitiveOption);
    const QRegularExpressionMatch match = re.match(text);
    if (!match.hasMatch()) {
        return invalidOutputPowerValue();
...
static double outputPowerRoleDouble(QTableWidgetItem *item, int role, double fallback)
{
    if (!item) {
        return fallback;
    }
...
static QString outputPowerRoleString(QTableWidgetItem *item, int role, const QString &fallback)
{
    if (!item) {
        return fallback;
    }
...
static void setRoleDoubleIfValid(QTableWidgetItem *item, int role, double value)
{
    if (!item) {
        return;
    }
    if (outputPowerIsValid(value)) {
...
static void tagOutputPowerRunRow(QTableWidget *table,
                                 int row,
                                 double targetPowerDbm,
                                 double measuredPowerDbm,
                                 double powerErrorDb,
                                 double frequencyMHz,
                                 double peakFrequencyMHz,
                                 double freqErrorKHz,
                                 const QString &bandwidthText,
                                 const QString &sampleText)
{
    ...
    if (!anchor) {
        anchor = new QTableWidgetItem(QString::number(row + 1));
        table->setItem(row, 0, anchor);
    }

    setRoleDoubleIfValid(anchor, RoleOutputTargetPowerDbm, targetPowerDbm);
    setRoleDoubleIfValid(anchor, RoleOutputMeasuredPowerDbm, measuredPowerDbm);
    setRoleDoubleIfValid(anchor, RoleOutputPowerErrorDb, powerErrorDb);
    setRoleDoubleIfValid(anchor, RoleOutputFrequencyMHz, frequencyMHz);
    setRoleDoubleIfValid(anchor, RoleOutputPeakFrequencyMHz, peakFrequencyMHz);
    setRoleDoubleIfValid(anchor, RoleOutputFrequencyErrorKHz, freqErrorKHz);
    anchor->setData(RoleOutputBandwidthText, bandwidthText);
    anchor->setData(RoleOutputSampleText, sampleText);

    for (int col = 0; col < table->columnCount(); ++col) {
        if (QTableWidgetItem *item = table->item(row, col)) {
            item->setData(RoleFullText, item->text());
...
static int preferredOutputPowerRow(QTableWidget *table, int requestedRow)
{
    if (!table || table->rowCount() <= 0) {
        return -1;
    }
    if (requestedRow >= 0 && requestedRow < table->rowCount()) {
        return requestedRow;
    }
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        const QString status = itemTextAt(table, row, 4).trimmed();
        if (!status.isEmpty() && status != QStringLiteral("WAIT")) {
            return row;
        }
    }
    return 0;
...
static QFrame *createOutputPowerStatusCard(QWidget *parent,
                                           const QString &title,
                                           const QString &value,
                                           const QString &unit)
{
    QFrame *card = new QFrame(parent);
    card->setObjectName(QStringLiteral("outputPowerStatusCard"));
    card->setMinimumHeight(74);
    card->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    card->setStyleSheet(QStringLiteral(
        "QFrame#outputPowerStatusCard { background: #ffffff; border: 1px solid #d8e2ef; border-radius: 8px; }"
        "QLabel#outputPowerIcon { background: #5c7fc7; color: white; border-radius: 13px; font-weight: bold; }"
        "QLabel#outputPowerCardTitle { color: #51647d; font-size: 11px; }"
        "QLabel#outputPowerCardValue { color: #213047; font-size: 16px; font-weight: 700; }"
        "QLabel#outputPowerCardUnit { color: #62728a; font-size: 11px; }"));
    ...
    icon->setObjectName(QStringLiteral("outputPowerIcon"));
    ...
    titleLabel->setObjectName(QStringLiteral("outputPowerCardTitle"));
    titleLabel->setWordWrap(true);
    textLayout->addWidget(titleLabel);

    QLabel *valueLabel = new QLabel(value, card);
    valueLabel->setObjectName(QStringLiteral("outputPowerCardValue"));
    valueLabel->setWordWrap(true);
    textLayout->addWidget(valueLabel);

    QLabel *unitLabel = new QLabel(unit, card);
    unitLabel->setObjectName(QStringLiteral("outputPowerCardUnit"));
    unitLabel->setWordWrap(true);
    textLayout->addWidget(unitLabel);

    layout->addLayout(textLayout, 1);
    return card;
}

static void updateOutputPowerStatusPanel(QGroupBox *box, QTableWidget *table, int requestedRow = -1)
{
    QVBoxLayout *layout = resetChartBoxLayout(box, QStringLiteral("仪表状态 - 当前功率点"));
    if (!box || !layout) {
        return;
    }
    ...
    const int row = preferredOutputPowerRow(table, requestedRow);
    if (row < 0) {
        QLabel *emptyLabel = new QLabel(QStringLiteral("等待 8.2 测试数据。开始测试后，点击下方表格任意一行可查看该功率点的仪表状态。"), box);
        emptyLabel->setObjectName(QStringLiteral("chartSubtitle"));
        emptyLabel->setWordWrap(true);
        layout->addWidget(emptyLabel, 1);
        return;
    }

    const int totalRows = table ? table->rowCount() : 0;
    const QString testPointText = itemTextAt(table, row, 1, QStringLiteral("功率点"));
    const QString configText = itemTextAt(table, row, 2, QStringLiteral("待配置"));
    const QString measureText = itemTextAt(table, row, 3, QStringLiteral("待测量"));
    const QString statusText = itemTextAt(table, row, 4, QStringLiteral("WAIT"));

    QTableWidgetItem *anchor = table ? table->item(row, 0) : nullptr;
    double targetPower = outputPowerRoleDouble(anchor, RoleOutputTargetPowerDbm,
                                               firstNumberWithUnit(testPointText + QStringLiteral(" ") + configText, QStringLiteral("dBm")));
    double measuredPower = outputPowerRoleDouble(anchor, RoleOutputMeasuredPowerDbm,
                                                 firstNumberWithUnit(measureText, QStringLiteral("dBm")));
    double powerError = outputPowerRoleDouble(anchor, RoleOutputPowerErrorDb,
                                              firstNumberAfterKeyword(measureText, QStringLiteral("误差"), QStringLiteral("dB")));
    double frequencyMHz = outputPowerRoleDouble(anchor, RoleOutputFrequencyMHz,
                                                firstNumberWithUnit(configText, QStringLiteral("MHz")));
    double peakFrequencyMHz = outputPowerRoleDouble(anchor, RoleOutputPeakFrequencyMHz,
                                                    firstNumberAfterKeyword(measureText, QStringLiteral("峰值"), QStringLiteral("MHz")));
    double freqErrorKHz = outputPowerRoleDouble(anchor, RoleOutputFrequencyErrorKHz,
                                                firstNumberAfterKeyword(measureText, QStringLiteral("频误"), QStringLiteral("kHz")));
    const double parsedBandwidthKHz = firstNumberWithUnit(configText, QStringLiteral("kHz"));
    const QString bandwidthText = outputPowerRoleString(anchor, RoleOutputBandwidthText,
                                                        outputPowerIsValid(parsedBandwidthKHz)
                                                        ? QString("%1 kHz").arg(parsedBandwidthKHz, 0, 'f', 0)
                                                        : QStringLiteral("-- kHz"));
    const QString sampleText = outputPowerRoleString(anchor, RoleOutputSampleText, QStringLiteral("--"));

    if (!outputPowerIsValid(peakFrequencyMHz) && outputPowerIsValid(frequencyMHz)) {
        peakFrequencyMHz = frequencyMHz;
    }

    QLabel *hintLabel = new QLabel(QStringLiteral("点击下方表格行后，这里显示该行的配置、测量和判定信息。"), box);
    hintLabel->setObjectName(QStringLiteral("chartSubtitle"));
    hintLabel->setWordWrap(true);
    hintLabel->setMaximumHeight(38);
    layout->addWidget(hintLabel);

    QWidget *gridWidget = new QWidget(box);
    QGridLayout *grid = new QGridLayout(gridWidget);
    grid->setContentsMargins(0, 0, 0, 0);
    grid->setHorizontalSpacing(10);
    grid->setVerticalSpacing(8);

    const QString pointValue = QString("%1 / %2").arg(row + 1).arg(qMax(totalRows, 1));
    const QString targetValue = outputPowerIsValid(targetPower)
            ? QString("%1 / %2").arg(targetPower, 0, 'f', 1).arg(OUTPUT_POWER_MAX_DBM, 0, 'f', 0)
            : QStringLiteral("-- / --");
    const QString errorValue = statusText.isEmpty() ? QStringLiteral("WAIT") : statusText;
    QString errorUnit;
    if (outputPowerIsValid(powerError) || outputPowerIsValid(freqErrorKHz)) {
        errorUnit = QStringLiteral("功率误差 %1 dB  频率误差 %2 kHz")
                .arg(outputPowerNumberText(powerError, 2), outputPowerNumberText(freqErrorKHz, 2));
    } else {
        errorUnit = QStringLiteral("功率误差 ≤3 dB，频率误差 ≤5 kHz");
    }

    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("测试点"),
                                                pointValue,
                                                testPointText), 0, 0);
    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("设定频率"),
                                                outputPowerNumberText(frequencyMHz, 6),
                                                QStringLiteral("MHz")), 0, 1);
    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("设定功率"),
                                                targetValue,
                                                QStringLiteral("dBm")), 0, 2);
    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("4071峰值频率"),
                                                outputPowerNumberText(peakFrequencyMHz, 6),
                                                QStringLiteral("MHz")), 1, 0);
    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("4071输出功率"),
                                                outputPowerNumberText(measuredPower, 2),
                                                QStringLiteral("dBm")), 1, 1);
    grid->addWidget(createOutputPowerStatusCard(gridWidget,
                                                QStringLiteral("判定信息"),
                                                errorValue,
                                                errorUnit), 1, 2);
    ...
    QLabel *statusLine = new QLabel(QStringLiteral("当前状态：<b>%1</b>　采样：%2　带宽：%3")
                                    .arg(statusText.isEmpty() ? QStringLiteral("WAIT") : statusText,
                                         sampleText,
                                         bandwidthText), box);
    statusLine->setTextFormat(Qt::RichText);
    statusLine->setObjectName(QStringLiteral("chartLegendLabel"));
    statusLine->setWordWrap(true);
    layout->addWidget(statusLine);
}

static bool outputPowerSummaryKeepWidget(QWidget *widget, const QList<QWidget *> &keepWidgets)
{
    return widget && keepWidgets.contains(widget);
}

static void clearSummaryLayoutKeepValues(QLayout *layout, const QList<QWidget *> &keepWidgets)
{
    if (!layout) {
        return;
    }
    ...
    while (QLayoutItem *item = layout->takeAt(0)) {
        if (QLayout *childLayout = item->layout()) {
            clearSummaryLayoutKeepValues(childLayout, keepWidgets);
            delete childLayout;
        }
        if (QWidget *widget = item->widget()) {
            if (outputPowerSummaryKeepWidget(widget, keepWidgets)) {
                widget->hide();
...
static QFrame *createOutputPowerSummaryTile(QWidget *parent,
                                            const QString &title,
                                            QLabel *valueLabel)
{
    QFrame *tile = new QFrame(parent);
    tile->setObjectName(QStringLiteral("outputPowerSummaryTile"));
    ...
    tileLayout->setContentsMargins(18, 10, 18, 10);
    tileLayout->setSpacing(5);

    QLabel *titleLabel = new QLabel(title, tile);
    titleLabel->setObjectName(QStringLiteral("outputPowerSummaryTileTitle"));
    ...
    if (valueLabel) {
        ...
        tileLayout->addWidget(valueLabel, 1);
    }

    return tile;
}

static void applyOutputPowerSummaryLayout(QGroupBox *summaryGroup,
                                           QLabel *caseValue,
                                           QLabel *freqValue,
                                           QLabel *bwValue)
{
    if (!summaryGroup || !caseValue || !freqValue || !bwValue) {
        return;
    }

    summaryGroup->setObjectName(QStringLiteral("summaryGroup"));
    caseValue->setObjectName(QStringLiteral("currentCaseValue"));
    freqValue->setObjectName(QStringLiteral("freqValue"));
    bwValue->setObjectName(QStringLiteral("bwValue"));

    summaryGroup->setTitle(QStringLiteral("当前测试摘要"));
    summaryGroup->setMinimumHeight(118);
    ...
    summaryGroup->setStyleSheet(QStringLiteral(
        ...
        "  background: #f4f8fd;"
        "}"
        "QFrame#outputPowerSummaryTile {"
        "  background: #f8fbff;"
        "  border: 1px solid #dbe6f3;"
        "  border-radius: 8px;"
        "}"
        "QLabel#outputPowerSummaryTileTitle {"
        "  color: #5a6b84;"
        "  font-size: 14px;"
        "  font-weight: 600;"
        "}"
        "QLabel#currentCaseValue, QLabel#freqValue, QLabel#bwValue {"
        "  color: #14243b;"
        "  font-size: 23px;"
        "  font-weight: 800;"
        "}"
    ...
    if (summaryGroup->findChild<QWidget *>(QStringLiteral("outputPowerSummaryTilesWidget"), Qt::FindDirectChildrenOnly)) {
        return;
    }

    const QList<QWidget *> keepWidgets = {caseValue, freqValue, bwValue};
    for (QWidget *keepWidget : keepWidgets) {
        if (keepWidget) {
            keepWidget->hide();
            keepWidget->setParent(nullptr);
        }
    }
    if (QLayout *oldLayout = summaryGroup->layout()) {
        clearSummaryLayoutKeepValues(oldLayout, keepWidgets);
        delete oldLayout;
    }

    QVBoxLayout *rootLayout = new QVBoxLayout(summaryGroup);
    rootLayout->setContentsMargins(16, 12, 16, 14);
    rootLayout->setSpacing(0);

    QWidget *tilesWidget = new QWidget(summaryGroup);
    tilesWidget->setObjectName(QStringLiteral("outputPowerSummaryTilesWidget"));
    tilesWidget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);

    QHBoxLayout *tilesLayout = new QHBoxLayout(tilesWidget);
    tilesLayout->setContentsMargins(0, 0, 0, 0);
    tilesLayout->setSpacing(12);
    tilesLayout->addWidget(createOutputPowerSummaryTile(tilesWidget, QStringLiteral("当前用例"), caseValue), 3);
    tilesLayout->addWidget(createOutputPowerSummaryTile(tilesWidget, QStringLiteral("载波频率"), freqValue), 2);
    tilesLayout->addWidget(createOutputPowerSummaryTile(tilesWidget, QStringLiteral("信道带宽"), bwValue), 2);

    rootLayout->addWidget(tilesWidget, 1);
}

static void applyResultOverviewTypography(QLabel *verdictLabel, QLabel *statsLabel)
{
    // 只调整字体大小和字重，不改颜色，保留界面原有 PASS / WAIT / FAIL 配色。
    if (verdictLabel) {
        verdictLabel->setMinimumWidth(160);
        verdictLabel->setAlignment(Qt::AlignCenter);
        QFont verdictFont = verdictLabel->font();
        verdictFont.setPointSize(30);
    ...
    if (statsLabel) {
        ...
        statsFont.setWeight(QFont::DemiBold);
        statsLabel->setFont(statsFont);
    }
}

static QString outputPowerOverviewText(int passCount,
                                       int failCount,
                                       int waitCount,
                                       int totalCount,
                                       const QString &detail = QString())
{
    const int safeTotal = qMax(totalCount, passCount + failCount + waitCount);
    const int finishedCount = passCount + failCount;
    QString text = QStringLiteral("PASS：%1 项　FAIL：%2 项　WAIT：%3 项\n"
                                  "已完成：%4/%5　当前结果：%6")
            .arg(passCount)
            .arg(failCount)
            .arg(waitCount)
            .arg(finishedCount)
            .arg(safeTotal)
            .arg(failCount > 0 ? QStringLiteral("存在失败项")
                               : (waitCount > 0 ? QStringLiteral("等待测试完成")
                                                : QStringLiteral("全部通过")));
...
static void setOutputPowerResultOverview(QLabel *verdictLabel,
                                         QLabel *statsLabel,
                                         ...
                                         const QString &detail = QString())
{
    applyResultOverviewTypography(verdictLabel, statsLabel);
    if (verdictLabel) {
        verdictLabel->setText(verdict);
    }
    if (statsLabel) {
        statsLabel->setText(outputPowerOverviewText(passCount, failCount, waitCount, totalCount, detail));
    }
}
...
void MainWindow::enhanceUi()
{
    ...
    decorateTables();
    applyResultOverviewTypography(ui->verdictLabel, ui->statsLabel);
    ...
    connect(ui->resultTableRun, &QTableWidget::currentCellChanged,
        ...
        if (currentTestCase.contains("8.6")) {
            ...
            updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, currentRow);
        }
    });
    connect(ui->resultTableRun, &QTableWidget::cellClicked,
            this, [this](int row, int) {
        if (currentTestCase.contains("8.2")) {
            updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, row);
        }
    });
    connect(ui->resultTableRun, &QTableWidget::cellClicked,
...
void MainWindow::setupOutputPowerSchematicChart()
{
    ...
    QVBoxLayout *layout = resetChartBoxLayout(box, QStringLiteral("仪表状态—SCPI通信日志摘要"));
    if (!box || !layout) {
        return;
    }

    // 8.2 结果显示页左图：不再绘制链路示意图，改为截取日志表中的 SCPI 通信记录展示。
    // 右侧 groupBox_2 的实测功率时间图不动。
    box->setMinimumHeight(300);
    layout->setContentsMargins(14, 18, 14, 12);
    layout->setSpacing(8);

    QLabel *subtitle = new QLabel(QStringLiteral("截取测试日志中的仪表通信记录，用于核对 1466 下发、4071 查询及返回状态。"), box);
    subtitle->setObjectName(QStringLiteral("chartSubtitle"));
    subtitle->setWordWrap(true);
    subtitle->setMaximumHeight(40);
    layout->addWidget(subtitle);

    QTableWidget *scpiTable = new QTableWidget(box);
    scpiTable->setObjectName(QStringLiteral("outputPowerScpiSnapshotTable"));
    scpiTable->setColumnCount(5);
    scpiTable->setHorizontalHeaderLabels({QStringLiteral("时间"),
                                          QStringLiteral("仪表"),
                                          QStringLiteral("命令"),
                                          QStringLiteral("返回"),
                                          QStringLiteral("状态")});

    // 左侧日志区域使用真实 QTableWidget，不再用图片式绘制。
    // 样式尽量复用全局表格风格，保持与测试运行/结果表格一致。
    scpiTable->verticalHeader()->setVisible(false);
    scpiTable->horizontalHeader()->setVisible(true);
    scpiTable->setEditTriggers(QAbstractItemView::NoEditTriggers);
    scpiTable->setSelectionMode(QAbstractItemView::NoSelection);
    scpiTable->setSelectionBehavior(QAbstractItemView::SelectRows);
    scpiTable->setAlternatingRowColors(true);
    scpiTable->setShowGrid(false);
    scpiTable->setWordWrap(false);
    scpiTable->setTextElideMode(Qt::ElideRight);
    scpiTable->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    scpiTable->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    scpiTable->setMinimumHeight(210);
    scpiTable->verticalHeader()->setDefaultSectionSize(34);
    ...
    QList<ScpiRow> rows;
    auto addSnapshotRow = [&](const QString &time,
                              const QString &instrument,
                              const QString &command,
                              const QString &response,
                              const QString &status) {
        rows.append({time, instrument, command, response, status});
    };

    auto displayInstrumentName = [](const QString &source) {
        if (source.contains(QStringLiteral("4071"))) {
            return QStringLiteral("4071 频谱分析仪");
        }
        if (source.contains(QStringLiteral("1466"))) {
            return QStringLiteral("1466 信号发生器");
        }
        if (source.contains(QStringLiteral("8.2"))) {
            return QStringLiteral("8.2 测试流程");
        }
        return source.isEmpty() ? QStringLiteral("仪表") : source;
    };

    auto extractCommand = [](const QString &content) {
        const QStringList commandPatterns = {
            QStringLiteral("\\*IDN\\?"),
            QStringLiteral("\\*OPC\\?"),
            QStringLiteral(":SYST(?:em)?:ERR(?:or)?\\?"),
            QStringLiteral(":FREQ[^,;\\s]*"),
            QStringLiteral(":POW[^,;\\s]*"),
            QStringLiteral(":OUTP[^,;\\s]*"),
            QStringLiteral(":CALC[^,;\\s]*"),
            QStringLiteral(":READ[^,;\\s]*"),
            QStringLiteral(":INIT[^,;\\s]*"),
            QStringLiteral(":MARK[^,;\\s]*")
        };
        ...
        QString compact = content.simplified();
        if (compact.size() > 34) {
            compact = compact.left(31) + QStringLiteral("...");
        }
        return compact.isEmpty() ? QStringLiteral("--") : compact;
    };

    auto extractResponse = [](const QString &content, const QString &level) {
        if (level.compare(QStringLiteral("ERROR"), Qt::CaseInsensitive) == 0
            || level.compare(QStringLiteral("FAIL"), Qt::CaseInsensitive) == 0) {
            return QStringLiteral("error");
        }
        if (level.compare(QStringLiteral("PASS"), Qt::CaseInsensitive) == 0
            || level.compare(QStringLiteral("OK"), Qt::CaseInsensitive) == 0) {
            return QStringLiteral("done");
        }
        const QRegularExpression responseRe(QStringLiteral("(?:返回|响应|response|reply)[:：]?\\s*([^;，,]+)"),
                                            QRegularExpression::CaseInsensitiveOption);
        const QRegularExpressionMatch match = responseRe.match(content);
        if (match.hasMatch()) {
            QString response = match.captured(1).trimmed();
            if (response.size() > 30) {
                response = response.left(27) + QStringLiteral("...");
            }
            return response;
        }
    ...
    if (ui->logTable && ui->logTable->rowCount() > 0) {
        const int maxRows = 6;
        const int startRow = qMax(0, ui->logTable->rowCount() - maxRows);
        for (int row = startRow; row < ui->logTable->rowCount(); ++row) {
            const QString time = itemTextAt(ui->logTable, row, 0, QStringLiteral("--"));
            const QString level = itemTextAt(ui->logTable, row, 1, QStringLiteral("WAIT"));
            const QString source = itemTextAt(ui->logTable, row, 2, QStringLiteral("仪表"));
            const QString content = itemTextAt(ui->logTable, row, 3, QStringLiteral("--"));
            addSnapshotRow(time,
                           displayInstrumentName(source),
                           extractCommand(content),
                           extractResponse(content, level),
                           level.isEmpty() ? QStringLiteral("WAIT") : level);
        }
    }

    if (rows.isEmpty()) {
        addSnapshotRow(QStringLiteral("10:15:10.123"), QStringLiteral("4071 频谱分析仪"), QStringLiteral("*IDN?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
        addSnapshotRow(QStringLiteral("10:15:10.456"), QStringLiteral("1466 信号发生器"), QStringLiteral("*IDN?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
        addSnapshotRow(QStringLiteral("10:15:10.789"), QStringLiteral("4071 频谱分析仪"), QStringLiteral(":SYST:ERR?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
        addSnapshotRow(QStringLiteral("10:15:11.012"), QStringLiteral("1466 信号发生器"), QStringLiteral(":SYST:ERR?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
        addSnapshotRow(QStringLiteral("10:15:11.345"), QStringLiteral("4071 频谱分析仪"), QStringLiteral("*OPC?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
        addSnapshotRow(QStringLiteral("10:15:11.678"), QStringLiteral("1466 信号发生器"), QStringLiteral("*OPC?"), QStringLiteral("pending"), QStringLiteral("WAIT"));
    }

    scpiTable->setRowCount(rows.size());
    int passCount = 0;
    int failCount = 0;
    for (int row = 0; row < rows.size(); ++row) {
        const ScpiRow &snapshot = rows[row];
        const QStringList values = {snapshot.time, snapshot.instrument, snapshot.command, snapshot.response, snapshot.status};
        for (int col = 0; col < values.size(); ++col) {
            QTableWidgetItem *item = makeTableItem(values[col]);
            if (col == 4) {
                const QString statusUpper = values[col].toUpper();
                if (statusUpper.contains(QStringLiteral("PASS")) || statusUpper.contains(QStringLiteral("OK"))) {
                    item->setForeground(QBrush(QColor("#16794c")));
                    ++passCount;
                } else if (statusUpper.contains(QStringLiteral("FAIL")) || statusUpper.contains(QStringLiteral("ERROR"))) {
                    item->setForeground(QBrush(QColor("#c92a2a")));
                    ++failCount;
                } else {
            ...
            scpiTable->setItem(row, col, item);
        }
    }

    // 按照主界面统一表格样式二次整理，同时保留日志表适合阅读的列宽。
    polishTable(scpiTable, true);
    scpiTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::Fixed);
    scpiTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Fixed);
    scpiTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::Stretch);
    scpiTable->horizontalHeader()->setSectionResizeMode(3, QHeaderView::Stretch);
    scpiTable->horizontalHeader()->setSectionResizeMode(4, QHeaderView::Fixed);
    scpiTable->setColumnWidth(0, 106);
    scpiTable->setColumnWidth(1, 132);
    scpiTable->setColumnWidth(4, 78);
    for (int row = 0; row < scpiTable->rowCount(); ++row) {
        for (int col = 0; col < scpiTable->columnCount(); ++col) {
            if (QTableWidgetItem *item = scpiTable->item(row, col)) {
                item->setTextAlignment(col == 2 || col == 3
    ...
    layout->addWidget(scpiTable, 1);

    QLabel *summaryLabel = new QLabel(QStringLiteral("统计信息：通过 %1 项 | 失败 %2 项 | 总计 %3 项 | 该区域显示最近 %4 条日志")
                                      .arg(passCount)
                                      .arg(failCount)
                                      .arg(rows.size())
                                      .arg(rows.size()), box);
    summaryLabel->setObjectName(QStringLiteral("chartLegendLabel"));
                                     ...
                                     bool running)
{
    ...
    // 动态图区域要给坐标轴和曲线留足空间，避免标题、图例、坐标轴挤在一起。
    box->setMinimumHeight(running ? 320 : 300);
    ...
    subtitle->setWordWrap(true);
    subtitle->setMaximumHeight(44);
    layout->addWidget(subtitle);

    QChartView *chartView = new QChartView(box);
    chartView->setRenderHint(QPainter::Antialiasing);
    chartView->setMinimumHeight(running ? 270 : 250);
    chartView->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);

    QChart *chart = new QChart();
    // groupBox 已经显示标题，这里不再设置 chart title，防止刷新时标题区域过高造成文字重叠。
    ...
    // 纵轴单位放到下方说明里，不再使用竖排轴标题，避免和左侧刻度数字重叠。
    axisY->setTitleText(QString());
    if (qAbs(maxY - minY) < 0.001) {
        minY -= 1.0;
        maxY += 1.0;
    }
    ...
    QLabel *legend = new QLabel(running
                                ? "蓝线：4071实测功率；虚线：配置功率。单位：dBm。"
                                : "完整采样结果：蓝线为4071实测功率，虚线为配置功率，单位：dBm。",
                                box);
...
void MainWindow::updateOutputPowerRunLiveChart(const QVector<double> &timeSec,
                                               ...
                                               bool running)
{
    // 8.2 测试运行页: 左侧显示当前表格行的仪表状态卡片，右侧显示实时功率动态图。
    updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"),
                                 ui->resultTableRun,
                                 ui->resultTableRun ? ui->resultTableRun->currentRow() : -1);

    QGroupBox *liveBox = findChild<QGroupBox*>("groupBox_4");
    if (liveBox) {
        liveBox->show();
    }
    populateOutputPowerChart(liveBox,
                             "实时功率动态曲线",
                             "8.2 功率采样动态显示：右侧实时刷新4071实测功率与配置功率。",
                             timeSec,
                             measuredPowerDbm,
                             targetPowerDbm,
                             running);
...
void MainWindow::applyCaseResultTemplate(const QString &caseName)
{
    ...
    if (caseName.contains("8.2")) {
        bool runPointsOk = false;
        QString runPointsError;
        QList<double> runPowerPoints = outputPowerTestPoints(&runPointsOk, &runPointsError);
        if (!runPointsOk || runPowerPoints.isEmpty()) {
            runPowerPoints.clear();
            runPowerPoints << 0.0 << 3.0 << 7.5 << 12.0 << 15.0;
        }
        const QString runFreqText = QString::number(ui->carrierFreqSpin->value(), 'f', 3) + " MHz";
        const QString runBwText = ui->bwCombo->currentText().trimmed() + " kHz";
        runTable->setColumnCount(5);
        runTable->setHorizontalHeaderLabels({"序号", "测试点", "配置", "测量", "状态"});
        runTable->setRowCount(runPowerPoints.size());
        for (int row = 0; row < runPowerPoints.size(); ++row) {
            const double point = runPowerPoints[row];
            runTable->setItem(row, 0, new QTableWidgetItem(QString::number(row + 1)));
            runTable->setItem(row, 1, new QTableWidgetItem(QString("功率点 %1 dBm").arg(point, 0, 'f', 2)));
            runTable->setItem(row, 2, new QTableWidgetItem(QString("1466 %1 dBm / %2 / %3")
                                                           .arg(point, 0, 'f', 2)
                                                           .arg(runFreqText)
                                                           .arg(runBwText)));
            runTable->setItem(row, 3, new QTableWidgetItem("4071 待测"));
            runTable->setItem(row, 4, new QTableWidgetItem("WAIT"));
            tagOutputPowerRunRow(runTable, row, point, invalidOutputPowerValue(), invalidOutputPowerValue(),
                                 ui->carrierFreqSpin->value(), invalidOutputPowerValue(), invalidOutputPowerValue(),
                                 runBwText, QStringLiteral("0/%1").arg(OUTPUT_POWER_SAMPLE_COUNT));
        }
        updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), runTable, 0);
        ...
        setOutputPowerResultOverview(ui->verdictLabel,
                                     ui->statsLabel,
                                     QStringLiteral("WAIT"),
                                     0,
                                     0,
                                     runPowerPoints.size(),
                                     runPowerPoints.size(),
                                     QStringLiteral("点击开始测试后，系统会逐个功率点刷新 PASS / FAIL 数量。"));
        return;
    }
...
void MainWindow::applyCaseTemplate(const QString &caseName)
{
    ...
    if (caseName.contains("8.2")) {
        ...
        applyOutputPowerSummaryLayout(ui->summaryGroup, ui->currentCaseValue, ui->freqValue, ui->bwValue);
    ...
    if (currentTestCase.contains("8.1")) {
        ...
        const QString runFreqText = QString::number(ui->carrierFreqSpin->value(), 'f', 3) + " MHz";
        const QString runBwText = ui->bwCombo->currentText().trimmed() + " kHz";
        ui->resultTableRun->setColumnCount(5);
        ui->resultTableRun->setHorizontalHeaderLabels({"序号", "测试点", "配置", "测量", "状态"});
        ui->resultTableRun->setRowCount(runPowerPoints.size());
        for (int row = 0; row < runPowerPoints.size(); ++row) {
            const double point = runPowerPoints[row];
            ui->resultTableRun->setItem(row, 0, new QTableWidgetItem(QString::number(row + 1)));
            ui->resultTableRun->setItem(row, 1, new QTableWidgetItem(QString("功率点 %1 dBm").arg(point, 0, 'f', 2)));
            ui->resultTableRun->setItem(row, 2, new QTableWidgetItem(QString("1466 %1 dBm / %2 / %3")
                                                                     .arg(point, 0, 'f', 2)
                                                                     .arg(runFreqText)
                                                                     .arg(runBwText)));
            ui->resultTableRun->setItem(row, 3, new QTableWidgetItem("4071 待测"));
            ui->resultTableRun->setItem(row, 4, new QTableWidgetItem("WAIT"));
            tagOutputPowerRunRow(ui->resultTableRun, row, point, invalidOutputPowerValue(), invalidOutputPowerValue(),
                                 ui->carrierFreqSpin->value(), invalidOutputPowerValue(), invalidOutputPowerValue(),
                                 runBwText, QStringLiteral("0/%1").arg(OUTPUT_POWER_SAMPLE_COUNT));
        }
        updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, 0);
...
bool MainWindow::runRealOutputPowerLinkTest()
{
    ...
    if (!pointsOk) {
        ...
        setOutputPowerResultOverview(ui->verdictLabel,
                                     ...
                                     0,
                                     1,
                                     QStringLiteral("功率点配置错误：") + pointsError);
        return false;
    }
    ...
    for (int row = 0; row < powerPoints.size(); ++row) {
        const double point = powerPoints[row];
        ui->resultTableRun->setItem(row, 0, new QTableWidgetItem(QString::number(row + 1)));
        ui->resultTableRun->setItem(row, 1, new QTableWidgetItem(QString("功率点 %1 dBm").arg(point, 0, 'f', 2)));
        ui->resultTableRun->setItem(row, 2, new QTableWidgetItem(QString("1466 %1 dBm / %2 MHz / %3 kHz")
                                                                 .arg(point, 0, 'f', 2)
        ...
        ui->resultTableRun->setItem(row, 4, new QTableWidgetItem("WAIT"));
        tagOutputPowerRunRow(ui->resultTableRun, row, point, invalidOutputPowerValue(), invalidOutputPowerValue(),
                             frequencyMHz, invalidOutputPowerValue(), invalidOutputPowerValue(),
                             bandwidth + QStringLiteral(" kHz"), QStringLiteral("0/%1").arg(OUTPUT_POWER_SAMPLE_COUNT));
    }
    ...
    setOutputPowerResultOverview(ui->verdictLabel,
                                 ...
                                 powerPoints.size(),
                                 powerPoints.size(),
                                 QStringLiteral("真实仪表联调准备中：1466 下发功率，4071 采样测量。"));
    ...
    for (int row = 0; row < powerPoints.size(); ++row) {
        ...
        ui->resultTableRun->selectRow(row);
        updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, row);
        QCoreApplication::processEvents();
        ...
        ui->resultTableRun->setItem(row, 0, new QTableWidgetItem(QString::number(row + 1)));
        ui->resultTableRun->setItem(row, 1, new QTableWidgetItem(QString("功率点 %1 dBm").arg(targetPowerDbm, 0, 'f', 2)));
        ui->resultTableRun->setItem(row, 2, new QTableWidgetItem(genOk
                                                                 ? QString("1466 %1 dBm / %2 MHz / %3 kHz")
                                                                       .arg(targetPowerDbm, 0, 'f', 2)
                                                                       .arg(frequencyMHz, 0, 'f', 3)
                                                                       .arg(bandwidth)
                                                                 : "1466 配置失败"));
        ui->resultTableRun->setItem(row, 3, new QTableWidgetItem(peakOk
                                                                 ? QString("4071 %1 dBm / 误差 %2 dB / 峰值 %3 MHz / 频误 %4 kHz")
                                                                       .arg(avgPowerDbm, 0, 'f', 2)
                                                                       .arg(powerErrorDb, 0, 'f', 2)
                                                                       .arg(avgFreqMHz, 0, 'f', 6)
                                                                       .arg(freqErrorKHz, 0, 'f', 2)
                                                                 : (failureReason.isEmpty() ? QString("4071 未返回") : failureReason)));
        ui->resultTableRun->setItem(row, 4, new QTableWidgetItem(verdict));
        tagOutputPowerRunRow(ui->resultTableRun, row, targetPowerDbm,
                             peakOk ? avgPowerDbm : invalidOutputPowerValue(),
                             peakOk ? powerErrorDb : invalidOutputPowerValue(),
                             frequencyMHz,
                             peakOk ? avgFreqMHz : invalidOutputPowerValue(),
                             peakOk ? freqErrorKHz : invalidOutputPowerValue(),
                             bandwidth + QStringLiteral(" kHz"),
                             QString("%1/%2").arg(pointPowers.size()).arg(OUTPUT_POWER_SAMPLE_COUNT));
        updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, row);
        ...
        const int waitCount = qMax(0, powerPoints.size() - passCount - failCount);
        const QString liveVerdict = failCount > 0
                ? QStringLiteral("FAIL")
                : (waitCount > 0 ? QStringLiteral("WAIT") : QStringLiteral("PASS"));
        setOutputPowerResultOverview(ui->verdictLabel,
                                     ui->statsLabel,
                                     liveVerdict,
                                     passCount,
                                     ...
                                     waitCount,
                                     powerPoints.size(),
                                     QStringLiteral("当前功率点：%1 dBm；最大功率误差：%2 dB")
                                     .arg(targetPowerDbm, 0, 'f', 2)
    ...
    setOutputPowerResultOverview(ui->verdictLabel,
                                 ui->statsLabel,
                                 overallPass ? QStringLiteral("PASS") : QStringLiteral("FAIL"),
                                 passCount,
                                 failCount,
                                 0,
                                 powerPoints.size(),
                                 QStringLiteral("最大功率误差：%1 dB；门限：±%2 dB%3")
                                 .arg(maxAbsPowerErrorDb, 0, 'f', 2)
                                 .arg(OUTPUT_POWER_TOLERANCE_DB, 0, 'f', 2)
                                 .arg(analyzerLimitTriggered ? QStringLiteral("；4071输入保护已触发") : QString()));
...
void MainWindow::runTest_8_2()
{
    ...
    if (!timeout.isActive()) {
        ...
        setOutputPowerResultOverview(ui->verdictLabel,
                                     ...
                                     0,
                                     1,
                                     QStringLiteral("测量超时：未在规定时间内收到仪表读数。"));
        return;
    }
    ...
    // 更新测试运行页表格：保持“序号 / 测试点 / 配置 / 测量 / 状态”格式
    ui->resultTableRun->clearContents();
    ui->resultTableRun->setColumnCount(5);
    ui->resultTableRun->setHorizontalHeaderLabels({"序号", "测试点", "配置", "测量", "状态"});
    ui->resultTableRun->setRowCount(3);
    ui->resultTableRun->setItem(0, 0, new QTableWidgetItem("1"));
    ui->resultTableRun->setItem(0, 1, new QTableWidgetItem("输出功率"));
    ui->resultTableRun->setItem(0, 2, new QTableWidgetItem(QString("设定 %1 dBm")
                                                           .arg(targetPower_dBm, 0, 'f', 1)));
    ui->resultTableRun->setItem(0, 3, new QTableWidgetItem(QString("实测 %1 dBm")
                                                           .arg(power_measured_dBm, 0, 'f', 2)));
    ui->resultTableRun->setItem(0, 4, new QTableWidgetItem(pass ? "PASS" : "FAIL"));

    ui->resultTableRun->setItem(1, 0, new QTableWidgetItem("2"));
    ui->resultTableRun->setItem(1, 1, new QTableWidgetItem("功率误差"));
    ui->resultTableRun->setItem(1, 2, new QTableWidgetItem(QString("允许 ±%1 dB")
                                                           .arg(OUTPUT_POWER_TOLERANCE_DB, 0, 'f', 2)));
    ui->resultTableRun->setItem(1, 3, new QTableWidgetItem(QString("误差 %1 dB")
                                                           .arg(error, 0, 'f', 2)));
    ui->resultTableRun->setItem(1, 4, new QTableWidgetItem(pass ? "PASS" : "FAIL"));

    ui->resultTableRun->setItem(2, 0, new QTableWidgetItem("3"));
    ui->resultTableRun->setItem(2, 1, new QTableWidgetItem("电压读数"));
    ui->resultTableRun->setItem(2, 2, new QTableWidgetItem("目标 " + QString::number(voltage_set, 'f', 2) + " V"));
    ui->resultTableRun->setItem(2, 3, new QTableWidgetItem("实测 " + QString::number(voltage_measured, 'f', 2) + " V"));
    ui->resultTableRun->setItem(2, 4, new QTableWidgetItem("—"));

    tagOutputPowerRunRow(ui->resultTableRun, 0, targetPower_dBm, power_measured_dBm, error,
                         frequency, frequency, 0.0, bandwidth, QStringLiteral("1/1"));
    tagOutputPowerRunRow(ui->resultTableRun, 1, targetPower_dBm, power_measured_dBm, error,
                         frequency, frequency, 0.0, bandwidth, QStringLiteral("1/1"));
    tagOutputPowerRunRow(ui->resultTableRun, 2, targetPower_dBm, invalidOutputPowerValue(), invalidOutputPowerValue(),
                         frequency, invalidOutputPowerValue(), invalidOutputPowerValue(), bandwidth, QStringLiteral("1/1"));
    updateOutputPowerStatusPanel(findChild<QGroupBox*>("groupBox_3"), ui->resultTableRun, 0);
    // 更新结果显示页
    setOutputPowerResultOverview(ui->verdictLabel,
                                 ui->statsLabel,
                                 ...
                                 0,
                                 2,
                                 QStringLiteral("测量功率：%1 dBm；功率误差：%2 dB")
                                 .arg(power_measured_dBm, 0, 'f', 2)
                                 .arg(error, 0, 'f', 2));
