...
static const double OUTPUT_POWER_MIN_DBM = Case82Constants::OutputPowerMinDbm;          // 8.2 当前现场验证范围
static const double OUTPUT_POWER_MAX_DBM = Case82Constants::OutputPowerMaxDbm;
static const double OUTPUT_POWER_ANALYZER_LIMIT_DBM = Case82Constants::AnalyzerLimitDbm;
static const double OUTPUT_POWER_TOLERANCE_DB = Case82Constants::PowerToleranceDb;
static const int OUTPUT_POWER_SAMPLE_COUNT = Case82Constants::SampleCount;
static const int OUTPUT_POWER_SAMPLE_INTERVAL_MS = Case82Constants::SampleIntervalMs;
    ...
    , lastGeneratorIdnResponse()
{
    ...
    connect(case82RunController, &Case82RunController::summaryReady,
            this, &MainWindow::showCase82Summary);
    connect(case82RunController, &Case82RunController::logReady,
            this, [this](const QString &level, const QString &source,
                         const QString &message) {
        addLog(QDateTime::currentDateTime().toString("hh:mm:ss"),
               level, source, message);
    });
    connect(case82RunController, &Case82RunController::initialReady,
            this, &MainWindow::prepareCase82ResultTables);
    connect(case82RunController, &Case82RunController::sampleReady,
            this, &MainWindow::presentCase82Sample);
    connect(case82RunController, &Case82RunController::rowReady,
            this, &MainWindow::presentCase82Row);
    connect(case82RunController, &Case82RunController::resultReady,
            this, &MainWindow::presentCase82Result);
    connect(case82RunController, &Case82RunController::stateChanged,
            this, [this](TestState state) {
        ui->statusBar->showMessage(
            QStringLiteral("8.2 状态: ") + testStateName(state));
        ui->pushButton->setEnabled(!case82RunController->isActive());
    });
    connect(case82RunController, &Case82RunController::cleanupCompleted,
            this, [this](bool safe) {
        addLog(QDateTime::currentDateTime().toString("hh:mm:ss"),
               safe ? "PASS" : "ERROR",
               "8.2",
    ...
    connect(case82RunController, &Case82RunController::finished,
            this, [this](const TestCompletion &completion) {
        testRunning = false;
        ui->pushButton->setEnabled(true);
        addLog(QDateTime::currentDateTime().toString("hh:mm:ss"),
...
void MainWindow::initComboBoxes()
{
    ...
    if (ui->levelFilterCombo) {
        const int debugIndex = ui->levelFilterCombo->findText("DEBUG", Qt::MatchFixedString);
        if (debugIndex < 0) {
            ui->levelFilterCombo->insertItem(1, "DEBUG");
        }
    }
...
void MainWindow::enhanceUi()
{
    ...
    connect(ui->centralTabs, &QTabWidget::currentChanged,
            this, [this](int index) {
        if (!currentTestCase.contains("8.5")) {
            return;
        }
        if (!ui->centralTabs || index < 0 || ui->centralTabs->widget(index) != ui->resultTab) {
            return;
        }
    ...
    if (styleFile.open(QFile::ReadOnly)) {
        QString styleSheet = QLatin1String(styleFile.readAll());
        // 1. 替换所有 "font-size: Xpt" 为 "font-size: 16pt"
        styleSheet.replace(QRegularExpression("font-size:\\s*\\d+pt"), "font-size:16pt");
        // 2. 替换所有 "font: Xpt" 为 "font: 16pt"
        styleSheet.replace(QRegularExpression("font:\\s*\\d+pt"), "font:16pt");
        // 3. 追加全局强制规则，覆盖 UI 文件中硬编码的 setFont
        styleSheet += " * { font-size: 16pt !important; }";
        this->setStyleSheet(styleSheet);
    } else {
        qWarning() << "无法加载样式表 style.qss";
    }
    // 单独强制参数配置页面的所有子控件字体为 16pt
        if (ui->scrollAreaWidgetContents) {
            ui->scrollAreaWidgetContents->setStyleSheet(
                "QWidget { font-size: 16pt !important; } "
                "QLabel { font-size: 16pt !important; } "
                "QGroupBox { font-size: 16pt !important; } "
                "QComboBox, QSpinBox, QDoubleSpinBox, QLineEdit { font-size: 16pt !important; } "
                "QPushButton { font-size: 16pt !important; } "
                "QCheckBox { font-size: 16pt !important; } "
                "QTextEdit { font-size: 16pt !important; }"
            );
        }
        // 强制测试运行页面的所有子控件字体为 16pt
        if (ui->testRunTab) {
            ui->testRunTab->setStyleSheet(
                "QWidget { font-size: 16pt !important; } "
                "QLabel { font-size: 16pt !important; } "
                "QGroupBox { font-size: 16pt !important; } "
                "QTableWidget { font-size: 16pt !important; } "
                "QPushButton { font-size: 16pt !important; } "
                "QPlainTextEdit { font-size: 16pt !important; } "
                "QProgressBar { font-size: 16pt !important; }"
            );
        }
}
...
QList<double> MainWindow::outputPowerTestPoints(bool *ok, QString *errorMessage) const
{
    ...
    return parseCase82PowerPoints(text, ok, errorMessage);
}
...
void MainWindow::applyCaseResultTemplate(const QString &caseName)
{
    ...
    if (caseName.contains("8.5")) {
        ...
        runTable->setRowCount(4);   // 示例4行

        const QStringList samplePoints = {
            "925MHz/200kHz/±300kHz",
            "925MHz/200kHz/±500kHz",
            "925MHz/400kHz/±500kHz",
            "925MHz/400kHz/±900kHz"
        };
        const QStringList sampleConfigs = {
            "IBW:180kHz",
            "IBW:180kHz",
            "IBW:360kHz",
            "IBW:360kHz"
        };
        const QStringList sampleMeas = {
            "L:--, R:--",
            "L:--, R:--",
            "L:--, R:--",
            "L:--, R:--"
        ...
        for (int i = 0; i < 4; ++i) {
            runTable->setItem(i, 0, new QTableWidgetItem(QString::number(i+1)));
            runTable->setItem(i, 1, new QTableWidgetItem(samplePoints[i]));
            runTable->setItem(i, 2, new QTableWidgetItem(sampleConfigs[i]));
            runTable->setItem(i, 3, new QTableWidgetItem(sampleMeas[i]));
            runTable->setItem(i, 4, new QTableWidgetItem("WAIT"));
        }
        ...
        resTable->setRowCount(4);

        const QStringList sampleConditions = {
            "925MHz/200kHz/±300kHz",
            "925MHz/200kHz/±500kHz",
            "925MHz/400kHz/±500kHz",
            "925MHz/400kHz/±900kHz"
        };
        const QStringList sampleItems = {
            "第一邻道ACLR",
            "第二邻道ACLR",
            "第一邻道ACLR",
            "第二邻道ACLR"
        };
        const QStringList sampleSet = {
            "IBW:180kHz",
            "IBW:180kHz",
            "IBW:360kHz",
            "IBW:360kHz"
        };
        const QStringList sampleStd = {
            "≥40.8 dB",
            "≥45.8 dB",
            "≥40.8 dB",
            "≥45.8 dB"
        ...
        for (int i = 0; i < 4; ++i) {
            resTable->setItem(i, 0, new QTableWidgetItem(QString::number(i+1)));
            resTable->setItem(i, 1, new QTableWidgetItem(sampleConditions[i]));
            resTable->setItem(i, 2, new QTableWidgetItem(sampleItems[i]));
            resTable->setItem(i, 3, new QTableWidgetItem(sampleSet[i]));
            resTable->setItem(i, 4, new QTableWidgetItem("待测"));
            resTable->setItem(i, 5, new QTableWidgetItem(sampleStd[i]));
            resTable->setItem(i, 6, new QTableWidgetItem("WAIT"));
        }
...
{
    if (ui->label_txPower) {
            ui->label_txPower->setText("发射功率(dBm)");
        }
    ...
    if (caseName.contains("8.7") || caseName == "8.7 ACS") {
        ...
        setupTextChartBox(findChild<QGroupBox*>("groupBox_3"),
                          "ACS 示意图",
                          "等待选择 8.7 测试点",
                          {"点击测试运行表任意行后，显示对应 schematic_0~7 示意图",
    ...
    if (caseName.contains("8.5")) {
        ...
        if (ui->label_txPower) {
                ui->label_txPower->setText("R2D信号功率(dBm)");
            }

        // ---------- 摘要栏 ----------
        ui->currentCaseValue->setText("8.5 ACLR");
        double freq = ui->carrierFreqSpin->value();
        QString bw = ui->bwCombo->currentText();
        ui->freqValue->setText(QString::number(freq, 'f', 1) + " MHz");
         ui->bwValue->setText("200/400/600/800 kHz");

        setCaseInfoPanel("8.5 ACLR / 邻道泄漏比",
                         "ACLR测试：配置载波、带宽、发射功率，遍历所有带宽和频点，测试运行页展示仿真图和结果截图。");
        setParamCheckPanel({"设备连接", "频点范围", "功率范围", "射频配置", "日志目录"},
                           {"OK", "OK", "OK", "CHECK", "INFO"},
                           {"仪表/基站在线", "由频点输入框设定", "由前端功率框决定", "待开始测试下发", "./log/merge_test*"});
        setDispatchQueuePanel({"SET:FREQ 频点", "SET:BW 带宽", "SET:POWER 前端值", "ARM:CAPTURE"},
                              {"已准备", "已准备", "待下发", "待下发"});
        setValidationMessages({"8.5 ACLR测试将遍历所有带宽和用户输入的频点",
                               "测试运行页表格中每行有“查看”按钮，点击显示对应的仿真图和结果截图",
                               "结果页柱状图展示各带宽的PASS/FAIL统计"});

        // 统一设置 groupBox 样式：固定高度，统一间距
        auto setupGroupBox = [](QGroupBox *box, const QString &title, int minH = 400, int maxH = 550) {
            if (!box) return;
            box->setTitle(title);
        ...
        if (simBox) {
            ...
            m_simImageLabel->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
            layout->addWidget(m_simImageLabel);
            simBox->setLayout(layout);
        }

        // ---------- 运行页：仪表测量状态（groupBox_4） ----------
        ...
        if (statusBox) {
            ...
            setupGroupBox(statusBox, "仪表测量状态");
            ...
            m_statusTextEdit->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
            m_statusTextEdit->setMinimumHeight(400);
            // 放大字体
            m_statusTextEdit->setStyleSheet("font-size: 23pt;");
        ...
        if (resultBox) {
            ...
            setupGroupBox(resultBox, "结果截图", 500, 650);
            resultBox->setMinimumWidth(0);
            resultBox->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
            m_resultScreenshotLabel = new AspectRatioPixmapLabel(resultBox);
            m_resultScreenshotLabel->setAlignment(Qt::AlignCenter);
            m_resultScreenshotLabel->setMinimumHeight(400);
            m_resultScreenshotLabel->setText("点击表格行查看截图");
            m_resultScreenshotLabel->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
        ...
        if (statBox) {
            ...
            setupGroupBox(statBox, "各带宽通过统计", 500, 650);
            statBox->setMinimumWidth(0);
            statBox->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
...
void MainWindow::connectToInstrument(const QString& ip, quint16 port)
{
    ...
    case82AnalyzerHost = ip.trimmed();
    case82AnalyzerPort = port;
...
void MainWindow::connectToSignalGenerator(const QString& ip, quint16 port)
{
    ...
    case82GeneratorHost = ip.trimmed();
    case82GeneratorPort = port;
...
void MainWindow::showCase82Summary(const QString &title,
                                   const QString &frequency,
                                   const QString &bandwidth,
                                   int progress,
                                   const QString &progressFormat)
...
void MainWindow::prepareCase82ResultTables(const Case82RunConfig &config)
{
    ui->resultTableRun->clearContents();
    ui->resultTableRun->setColumnCount(7);
    ui->resultTableRun->setHorizontalHeaderLabels({"功率点", "1466下发", "4071平均", "功率误差", "峰值频率", "频率误差", "判定"});
    ui->resultTableRun->setRowCount(config.powerPoints.size());

    ui->resultTableResult->clearContents();
    ui->resultTableResult->setColumnCount(6);
    ui->resultTableResult->setHorizontalHeaderLabels({"功率点", "平均功率", "功率误差", "采样数", "门限", "判定"});
    ui->resultTableResult->setRowCount(config.powerPoints.size());

    updateOutputPowerRunLiveChart(QVector<double>(), QVector<double>(), QVector<double>(), true);
    updateOutputPowerTrendChart(QVector<double>(), QVector<double>(), QVector<double>());
}

void MainWindow::presentCase82Sample(const Case82Sample &)
{
    // The full trend is rendered with the final result to avoid rebuilding
    // charts too aggressively from queued worker updates.
}

void MainWindow::presentCase82Row(int row, const Case82RowResult &result)
{
    const QString verdict = result.pass ? "PASS" : "FAIL";
    ui->resultTableRun->setItem(row, 0, new QTableWidgetItem(QString::number(result.targetPowerDbm, 'f', 2) + " dBm"));
    ui->resultTableRun->setItem(row, 1, new QTableWidgetItem(result.generatorConfigured ? QString::number(result.targetPowerDbm, 'f', 2) + " dBm" : "失败"));
    ui->resultTableRun->setItem(row, 2, new QTableWidgetItem(result.peakOk ? QString::number(result.averagePowerDbm, 'f', 2) + " dBm" : "未返回"));
    ui->resultTableRun->setItem(row, 3, new QTableWidgetItem(result.peakOk ? QString::number(result.powerErrorDb, 'f', 2) + " dB" : result.failureReason));
    ui->resultTableRun->setItem(row, 4, new QTableWidgetItem(result.peakOk ? QString::number(result.averageFrequencyMHz, 'f', 6) + " MHz" : "未返回"));
    ui->resultTableRun->setItem(row, 5, new QTableWidgetItem(result.peakOk ? QString::number(result.frequencyErrorKHz, 'f', 2) + " kHz" : "-"));
    ui->resultTableRun->setItem(row, 6, new QTableWidgetItem(verdict));

    ui->resultTableResult->setItem(row, 0, new QTableWidgetItem(QString::number(result.targetPowerDbm, 'f', 2) + " dBm"));
    ui->resultTableResult->setItem(row, 1, new QTableWidgetItem(result.peakOk ? QString::number(result.averagePowerDbm, 'f', 2) + " dBm" : "未返回"));
    ui->resultTableResult->setItem(row, 2, new QTableWidgetItem(result.peakOk ? QString::number(result.powerErrorDb, 'f', 2) + " dB" : result.failureReason));
    ui->resultTableResult->setItem(row, 3, new QTableWidgetItem(QString("%1/%2").arg(result.sampleCount).arg(OUTPUT_POWER_SAMPLE_COUNT)));
    ui->resultTableResult->setItem(row, 4, new QTableWidgetItem(QString("功率 ±%1 dB, 频率 ±5 kHz").arg(OUTPUT_POWER_TOLERANCE_DB, 0, 'f', 2)));
    ui->resultTableResult->setItem(row, 5, new QTableWidgetItem(verdict));
}

void MainWindow::presentCase82Result(const Case82Result &result)
{
    if (!result.generatorIdn.isEmpty()) {
        lastGeneratorIdnResponse = result.generatorIdn;
    }
    if (!result.analyzerIdn.isEmpty()) {
        lastIdnResponse = result.analyzerIdn;
    }

    updateOutputPowerRunLiveChart(result.trendTimeSec,
                                  result.trendMeasuredPowerDbm,
                                  result.trendTargetPowerDbm,
                                  false);
    updateOutputPowerTrendChart(result.trendTimeSec,
                                result.trendMeasuredPowerDbm,
                                result.trendTargetPowerDbm);
    ...
    ui->statsLabel->setText(QString("真实联调: 1466 输出 / 4071 测量\n"
                            ...
                            .arg(result.maxAbsPowerErrorDb, 0, 'f', 2)
                            .arg(OUTPUT_POWER_TOLERANCE_DB, 0, 'f', 2)
                            .arg(result.analyzerLimitTriggered ? "\n4071输入保护已触发" : ""));
    ui->progressBar->setValue(100);
}
...
void MainWindow::runTest_8_2()
{
    if (hasSpectrumAnalyzer() && hasSignalGenerator()) {
        ...
        config.powerPoints = powerPoints;
        config.frequencyMHz = ui->carrierFreqSpin->value();
        config.bandwidth = ui->bwCombo->currentText().trimmed();
        config.spanMHz = qMax(1.0, config.bandwidth.toDouble() / 1000.0 * 4.0);

        if (!case82RunController->start(config)) {
            addLog(QDateTime::currentDateTime().toString("hh:mm:ss"),
                   "WARN", "8.2", "当前运行尚未完成清理，不能重复启动");
        }
        return;
    }
...
void MainWindow::addLog(const QString &time, const QString &level, const QString &src, const QString &msg)
{
    ...
    if (normalizedLevel == "ERROR" || normalizedLevel == "FAIL") {
        ...
        fgColor = QColor("#667085");
        bgColor = QColor("#f2f4f7");
...
void MainWindow::onAcsTableRowClicked(int row, int column)
{
    ...
    const QString schematicPath = QString(":/images/sim/8_7/schematic_%1.png").arg(row);
...
void MainWindow::updateCase85BarChart(QGroupBox *targetBox)
{
    ...
    // 清空原有布局
    QLayout *oldLayout = targetBox->layout();
    if (oldLayout) {
        QLayoutItem *item;
    ...
    QVBoxLayout *layout = new QVBoxLayout(targetBox);
    targetBox->setTitle("各带宽通过统计");

    QTableWidget *runTable = ui->resultTableRun;
    if (!runTable || runTable->rowCount() == 0) {
        QLabel *emptyLabel = new QLabel("暂无测试数据", targetBox);
        emptyLabel->setAlignment(Qt::AlignCenter);
        layout->addWidget(emptyLabel);
        targetBox->setLayout(layout);
        return;
    }
    ...
    for (int row = 0; row < runTable->rowCount(); ++row) {
        QTableWidgetItem *testPointItem = runTable->item(row, 1);
        QTableWidgetItem *verdictItem = runTable->item(row, 4);
        if (!testPointItem || !verdictItem) continue;
        QString text = testPointItem->text();
        // 匹配 "200 kHz" 或 "200kHz"
        QRegularExpression re("\\b(\\d+)\\s*kHz\\b");
        ...
        if (!match.hasMatch()) {
            qDebug() << "No match for" << text;
            continue;
        }
        int bw = match.captured(1).toInt();
        bool pass = (verdictItem->text() == "PASS");
        if (!stats.contains(bw)) stats[bw] = qMakePair(0, 0);
        if (pass) stats[bw].first++;
        else stats[bw].second++;
    }

    if (stats.isEmpty()) {
        QLabel *emptyLabel = new QLabel("无有效数据", targetBox);
        emptyLabel->setAlignment(Qt::AlignCenter);
        layout->addWidget(emptyLabel);
        targetBox->setLayout(layout);
        return;
    ...
    // 创建柱状图
    QChartView *chartView = new QChartView(targetBox);
    chartView->setRenderHint(QPainter::Antialiasing);
    ...
    chart->setAnimationOptions(QChart::SeriesAnimations);  // 保留动画，但可改为 NoAnimation
    ...
    for (int bw : bws) {
        ...
        *failSet << stats[bw].second;
        maxPassVal = qMax(maxPassVal, stats[bw].first);
        maxFailVal = qMax(maxFailVal, stats[bw].second);
    }
    ...
    const int yMax = qMax(qMax(maxPassVal, maxFailVal), 1);
    axisY->setRange(0, yMax);
    axisY->setTickCount(yMax + 1);
...
void MainWindow::refresh85ResultTableColumnWidths()
{
    QTableWidget *table = ui->resultTableResult;
    if (!table) {
        return;
    }
    ...
    const int totalWidth = table->viewport() ? table->viewport()->width() : 0;
    if (totalWidth < 600) {
        return;
    }
    ...
    table->horizontalHeader()->setStretchLastSection(false);
    table->setColumnWidth(0, totalWidth * 0.05);
    table->setColumnWidth(1, totalWidth * 0.25);
    table->setColumnWidth(2, totalWidth * 0.15);
    table->setColumnWidth(3, totalWidth * 0.15);
    table->setColumnWidth(4, totalWidth * 0.15);
    table->setColumnWidth(5, totalWidth * 0.13);
    table->setColumnWidth(6, totalWidth * 0.12);
}

void MainWindow::onTestRowClicked(int row, int column)
{
    Q_UNUSED(column);
    if (row < 0 || row >= m_testRowData.size()) return;

    const TestRowData &data = m_testRowData[row];

    if (m_simImageLabel) {
        if (!data.simImagePath.isEmpty() && QFile::exists(data.simImagePath)) {
            QPixmap pix(data.simImagePath);
            if (!pix.isNull()) {
                m_simImageLabel->setPixmap(pix);
                m_simImageLabel->setText("");
            } else {
                m_simImageLabel->clear();
                m_simImageLabel->setText("仿真图加载失败");
            }
        } else {
            m_simImageLabel->clear();
        ...
        // 强制更新但不改变大小
        m_simImageLabel->update();
    }

    if (m_resultScreenshotLabel) {
        if (!data.screenshotPath.isEmpty() && QFile::exists(data.screenshotPath)) {
            QPixmap pix(data.screenshotPath);
            if (!pix.isNull()) {
                if (AspectRatioPixmapLabel *aspectLabel = dynamic_cast<AspectRatioPixmapLabel *>(m_resultScreenshotLabel)) {
                    aspectLabel->setSourcePixmap(pix);
                } else {
                    m_resultScreenshotLabel->setPixmap(pix);
                }
                m_resultScreenshotLabel->setText("");
            } else {
                m_resultScreenshotLabel->clear();
                m_resultScreenshotLabel->setText("截图加载失败");
            }
        } else {
            m_resultScreenshotLabel->clear();
            m_resultScreenshotLabel->setText("无截图");
        }
        m_resultScreenshotLabel->update();
    }

    if (m_statusTextEdit) {
        m_statusTextEdit->setPlainText(data.statusText);
